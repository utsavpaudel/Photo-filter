...
function upload(){
  ...
  orgImg = new SimpleImage(original);
  grayImg = new SimpleImage(original);
  redImg = new SimpleImage(original);
  randImg = new SimpleImage(original);
  rainbowImg = new SimpleImage(original);
  blurImg = new SimpleImage(original);
  orgImg.drawTo(canvas);
};

function randColor(){
  if (imageLoaded(randImg)==true){
    filterRand();
    randImg.drawTo(canvas);}  
};

function rainbowscale(){
  if (imageLoaded(rainbowImg)==true){
    filterRainbow();
    rainbowImg.drawTo(canvas);} 
}

function grayscale(){
  if (imageLoaded(grayImg)==true){
    filterGray();
    grayImg.drawTo(canvas);}
};

function redscale(){
  if (imageLoaded(redImg)==true){
    filterRed();
    redImg.drawTo(canvas);}
};

function blurscale(){
  if (imageLoaded(blurImg)==true){
    filterBlur();
...
function filterRand(){
  var randX = Math.ceil(Math.random()*randImg.width);
  var randY = Math.ceil(Math.random()*randImg.height);
  var target=randImg.getPixel(randX,randY);
  var targetRed= target.getRed();
  var targetBlue= target.getBlue();
  var targetGreen= target.getGreen();
  var outRed= Math.ceil(Math.random()*255);
  var outGreen= Math.ceil(Math.random()*255);
  var outBlue= Math.ceil(Math.random()*255);
  var margin = 30;
  document.getElementById("margin");
  for(var pixel of randImg.values()){
    if(Math.abs(pixel.getRed()-targetRed)<=margin && Math.abs(pixel.getGreen()-targetGreen)<=margin && Math.abs(pixel.getBlue()-targetBlue)<=margin){
...
function filterRainbow(){
  var w= rainbowImg.width;
  var h = rainbowImg.height;
  //var b calculates the width of each stripe
  var b = (w+h)/7;
  //var f is used to weight the location of the middle stripe
  var f = (-7*h*b)/(w+h);
...
function filterBlur(){
  ...
  for (var pixel of blurImg.values()){
    ...
    if (change>0.7){
      while (tempX>(orgImg.width-1)||tempX<0||tempY>(orgImg.height-1)||tempY<0){
      var tempX = x+Math.ceil((0.5-Math.random())*2*max);
      var tempY = y+Math.ceil((0.5-Math.random())*2*max)
      }
      blurImg.setPixel(x,y,orgImg.getPixel(tempX,tempY));
    }
    else{
